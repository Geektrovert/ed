...
// Use a simplified interface to avoid typing issues
export const HandsontableView = (props: any) => {
  const { node, updateAttributes, editor } = props;
  const hotRef = useRef<any>(null);
  const [isReady, setIsReady] = useState(false);
  const [tableTitle, setTableTitle] = useState(node.attrs.title || "Spreadsheet");
  ...
  const updateTableData = useCallback(() => {
    if (!hotRef.current?.hotInstance || isDestroyedRef.current || !isDirtyRef.current || isPendingUpdateRef.current || dataUpdateInProgressRef.current) {
      return;
    }
  ...
  const handleHotChange = useCallback((changes: any, source: string) => {
    if (!changes || source === 'loadData' || isInitialLoadRef.current || dataUpdateInProgressRef.current) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (tableData && Array.isArray(tableData)) {
      ...
      const safelyLoadData = () => {
        if (hotRef.current && hotRef.current.hotInstance && !isDestroyedRef.current) {
          try {
            ...
            console.error(`[HandsontableView ${tableId}] Error loading data:`, error);
            dataUpdateInProgressRef.current = false;
            isInitialLoadRef.current = false;
          }
        } else {
    ...
    const directHandleAfterChange = (changes: any[] | null, source: string) => {
    ...
    const directHandleFormulasUpdate = () => {
      ...
      try {
        ...
        console.error(`[HandsontableView ${tableId}] Error updating formulas:`, error);
      }
    };
    ...
    return () => {
      if (hot && !isDestroyedRef.current) {
        try {
          ...
          hot.removeHook('afterFormulasValuesUpdate', directHandleFormulasUpdate);
          ...
          console.error(`[HandsontableView ${tableId}] Error removing hooks:`, error);
        }
      }
    };
  }, [isReady, handleHotChange, handleStructureChange, tableData]);
  
  return (
    <NodeViewWrapper 
      className={cx(
        "handsontable-wrapper",
        "prose-lg w-full overflow-hidden"
      )}
    ...
    >
      ...
      <div className="text-xs text-muted-foreground bg-muted/10 border border-t-0 border-border rounded-b-md px-3 py-1.5 flex justify-end">
        <span>
          <kbd className="px-1.5 py-0.5 text-xs text-muted-foreground bg-muted rounded border border-border">Tab</kbd>
          {" "}to navigate • 
          {" "}<kbd className="px-1.5 py-0.5 text-xs text-muted-foreground bg-muted rounded border border-border">Right-click</kbd>
          {" "}for more options
        </span>
      </div>
    </NodeViewWrapper>
